    ...
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                ...
                try {
                    event = stripe_1.stripe.webhooks.constructEvent(body, signature, process.env.STRIPE_SECRET_KEY || '');
                }
                catch (error) {
                    return [2 /*return*/, res.status(400).send("Webhook Error: ".concat(error instanceof Error ? error.message : 'Unknown Error'))];
                }
                ;
            ...
            case 2:
                ...
                user = users[0];
                if (!user) {
                    return [2 /*return*/, res.status(400).json({ error: 'No such user exists.' })];
                }
                return [4 /*yield*/, payload.find({
                        collection: 'orders',
                        depth: 2,
            ...
            case 3:
                ...
                order = orders[0];
                if (!order) {
                    return [2 /*return*/, res.status(400).json({ error: 'No such order exists' })];
                }
                ;
